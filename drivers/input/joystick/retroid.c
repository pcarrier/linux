...
extern int qcom_spmi_haptics_rumble(unsigned int strong_magnitude,
				    unsigned int weak_magnitude);

static int retroid_rumble_play(struct input_dev *dev, void *data,
...
static int gamepad_mcu_uart_probe(struct serdev_device *serdev)
{
	...
	input_set_abs_params(gamepad_dev->dev_input, ABS_HAT2X, 0,
		trigger_left_max - trigger_left_antideadzone, 0, 30);
	input_set_abs_params(gamepad_dev->dev_input, ABS_HAT2Y, 0,
		trigger_right_max - trigger_right_antideadzone, 0, 30);

	input_set_capability(gamepad_dev->dev_input, EV_FF, FF_RUMBLE);
	ret = input_ff_create_memless(gamepad_dev->dev_input, NULL, retroid_rumble_play);
	if (ret)
		dev_warn(dev, "Failed to init rumble handler: %d\n", ret);
	...
	if (IS_ERR(gamepad_dev->platform_dev)) {
		...
		if (ret)
			dev_warn(dev, "Failed to create rumble_enable sysfs: %d\n", ret);
	}
